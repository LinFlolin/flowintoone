...
export function DashboardSidebar({ role, userName = "My account" }: DashboardSidebarProps) {
  ...
  return (
    <aside aria-label="Dashboard navigation" className="relative h-0 rounded-2xl border border-heather/20 bg-white/45 p-2 lg:h-auto lg:min-h-[calc(100vh-8.5rem)] lg:border-0 lg:bg-transparent lg:p-0">
      ...
      {isOpen && (
        <button type="button" aria-label="Close dashboard navigation" onClick={() => { setIsOpen(false); setMobileView("menu"); }} className="fixed inset-0 z-50 bg-ink/35 lg:hidden" />
      )}
      <div id="dashboard-mobile-drawer" className={`fixed inset-y-0 left-0 z-[55] w-[min(20rem,86vw)] overflow-y-auto bg-cream p-5 pt-24 shadow-[10px_0_35px_rgba(81,68,91,0.15)] transition-transform lg:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="absolute inset-x-5 top-0 flex h-20 items-center justify-between border-b border-heather/15">
          ...
          <button type="button" onClick={() => { setIsOpen(false); setMobileView("menu"); }} aria-label="Close dashboard navigation" className="grid size-9 place-items-center rounded-full border border-heather/60 text-xl font-light text-ink">×</button>
        </div>
        {mobileView === "menu" ? (
  ...
  return (
    <div className="pt-2">
      <div className="flex items-center gap-3 border-b border-heather/15 px-3 pb-4">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-ink">{userName}</p><Link href="/dashboard/profile" onClick={closeMenu} className="text-[10px] text-ink/55">View profile</Link></div>
      ...
      <div className="mt-3 grid gap-1">
        ...
        <button type="button" onClick={onDiscover} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-left text-xs font-semibold text-ink/80 hover:bg-heather/10"><DashboardIcon name="discover" className="size-4" />Discover<span className="ml-auto text-base text-ink/55">›</span></button>
        <Link href="/#events" onClick={closeMenu} className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/80 hover:bg-heather/10"><DashboardIcon name="calendar" className="size-4" />Events<span className="ml-auto text-base text-ink/55">›</span></Link>
        <div className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/50"><DashboardIcon name="profile" className="size-4" />About us<span className="ml-auto rounded-full bg-heather/10 px-2 py-1 text-[9px] text-heather">Coming soon</span></div>
      </div>

      <MobileMenuSection title="My space">
        ...
        {role === "artisan" && <Link href="/dashboard/storefront" onClick={closeMenu} className={itemClass("/dashboard/storefront")}><DashboardIcon name="storefront" className="size-4" />My website<span className="ml-auto text-base text-ink/55">›</span></Link>}
        <Link href="/dashboard/profile" onClick={closeMenu} className={itemClass("/dashboard/profile")}><DashboardIcon name="profile" className="size-4" />My profile<span className="ml-auto text-base text-ink/55">›</span></Link>
        {role === "artisan" && <div className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/50"><DashboardIcon name="settings" className="size-4" />Settings<span className="ml-auto text-[9px] uppercase tracking-wide text-ink/40">Soon</span></div>}
      </MobileMenuSection>

      <MobileMenuSection title="Support">
        <div className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/70"><DashboardIcon name="help" className="size-4" />Help &amp; FAQ<span className="ml-auto text-base text-ink/55">›</span></div>
        <div className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/70"><span className="text-base">✉</span>Contact us<span className="ml-auto text-[9px] uppercase tracking-wide text-ink/40">Soon</span></div>
        <form action={logoutAction}><button type="submit" className="flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-xs font-semibold text-ink/70 hover:bg-heather/10"><span className="text-base">↪</span>Logout</button></form>
      </MobileMenuSection>

      <div className="mt-4 flex items-center gap-3 rounded-xl bg-candy/10 px-3 py-3 text-[10px] text-ink/65"><span className="text-2xl text-candy" aria-hidden="true">◒</span><span>A kinder web<br />for creative people.</span></div>
    </div>
  );
}
...
function MobileDiscoverMenu({ queryHref, back, closeMenu }: { queryHref: string; back: () => void; closeMenu: () => void }) {
  const links = [
    { label: "All creators", href: queryHref, icon: "discover" as const },
    { label: "By category", href: queryHref, icon: "storefront" as const },
    { label: "By location", href: queryHref, icon: "discover" as const },
    { label: "Events", href: "/#events", icon: "calendar" as const },
  ];
  return (
    <div className="pt-2">
      <button type="button" onClick={back} className="flex min-h-10 items-center gap-2 px-1 text-xs font-semibold text-ink/65">← <span>Back to menu</span></button>
      <h2 className="mt-2 text-xl font-semibold tracking-[-0.04em] text-ink">Discover</h2>
      <div className="mt-3 grid gap-1">
        ...
        <div className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-ink/50"><DashboardIcon name="bookmark" className="size-4" />Collections<span className="ml-auto text-[9px] uppercase tracking-wide text-ink/40">Soon</span></div>
      </div>
      <p className="mt-5 border-t border-heather/15 pt-4 text-xs font-semibold text-ink/75">Quick links</p>
      ...
      <Link href="/discover" onClick={closeMenu} className="mt-4 flex items-center justify-between rounded-xl bg-heather/15 px-3 py-4 text-xs text-ink/75"><span><strong className="block text-sm text-ink">Discover<br />something special.</strong>Real people. Unique stories.</span><span className="grid size-8 place-items-center rounded-full bg-heather text-white">→</span></Link>
    </div>
  );
}
        ...
        {items.filter((item) => item.group === group).map((item) => {
          ...
          const content = (
            <>
              ...
              {!item.href && (
                <span className="ml-auto text-[9px] font-bold uppercase tracking-[0.1em] text-ink/40">Soon</span>
              )}
            </>
          );
          const className = `inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-semibold transition-colors ${isActive ? item.label === "Discover" ? "bg-viridian/15 text-viridian" : "bg-heather/15 text-heather" : item.href ? item.label === "Discover" ? "text-viridian/90 hover:bg-viridian/10 hover:text-viridian" : "text-ink/80 hover:bg-heather/10 hover:text-heather" : "cursor-default text-ink/55"}`;
