...
export function EventCalendar({ events }: { events: HomepageEvent[] }) {
  ...
  const selectedLabel = selectedDate ? new Intl.DateTimeFormat("en", { weekday: "long", month: "long", day: "numeric" }).format(selectedDate) : "Select a date";
  ...
  return <>
    <article className="h-full min-h-60 rounded-xl border border-heather/15 bg-white/70 p-4 lg:min-h-0 xl:p-5">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-semibold tracking-[-0.03em] text-ink xl:text-lg">Event calendar</h2>
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => moveMonth(-1)} aria-label="Previous month" className="grid size-7 place-items-center rounded-full text-heather hover:bg-heather/10 xl:size-9">‹</button>
          <span className="min-w-28 text-center text-xs font-semibold text-heather xl:text-sm">{formatMonth(visibleMonth)}</span>
          <button type="button" onClick={() => moveMonth(1)} aria-label="Next month" className="grid size-7 place-items-center rounded-full text-heather hover:bg-heather/10 xl:size-9">›</button>
        </div>
      </div>
      <div className="mt-4 grid grid-cols-7 text-center text-[9px] font-bold uppercase tracking-[0.08em] text-ink/45 xl:mt-5 xl:text-[10px]">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => <span key={day}>{day}</span>)}</div>
